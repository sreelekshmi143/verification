...
async def hide_pictures_handler(c: Client, m: Message):
    ...
    await m.delete()
@baboon.on_message(filters.video, group=3)
async def hide_videos_handler(c: Client, m: Message):
    hid_message = await m.forward(
        chat_id=bot_dustbin
    )
    ...
    await m.reply_text(
        f"I have hidden the video sent by {mention}.",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        text=f"{emoji.FRAMED_VIDEO} Show me the video.",
                        callback_data=f"shp_{hid_message.message_id}"
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=f"{emoji.FRAMED_PICTURE} Add the video to Chat.",
                        callback_data=f"apc_{hid_message.message_id}_{m.chat.id}"
                    )
                ]
            ]
...
async def apc_cb_handler(c: Client, cb: CallbackQuery):
    ...
    if len(cb_data) > 2:
        ...
        if cb.from_user.id in admin_list:
            ...
            await cb. answer(
                "You need to be an admin to approve this to be added to the chat permanently!",
                show_alert=True
            )
